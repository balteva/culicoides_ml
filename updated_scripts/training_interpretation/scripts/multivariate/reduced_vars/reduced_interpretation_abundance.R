...
multiv_model_abundance_LLO <- readRDS("./updated_scripts/models/reduced_RF_model_abundance_LLO_interpretation_clc.rds")


#CV with LTO
multiv_model_abundance_LTO <- readRDS("./updated_scripts/models/reduced_RF_model_abundance_LTO_interpretation_clc.rds")
...
plot_eval_abundance_model <- df_cv_abundance_LTO %>%
  ...
  dplyr::group_by(ECO_CLI,week, year) %>% #ECO_CLI
  ...
  facet_wrap(~ECO_CLI~year) + #~ECO_CLI #ncol=2 if only by year
  theme_bw() + 
  ...
  labs(color = expression("Number of " * italic("C. obsoletus") * " and " * italic("C. scoticus") * " by ecoclimatic zone (LTO CV)")) + 
...
plot_eval_abundance_model <- df_fin_cv %>%
  ...
  #geom_point() + 
  geom_line(linewidth = 0.8) + 
  facet_wrap(~year, ncol=2) + # ncol=2 #~ECO_CLI
  theme_bw()+
  ...
  xlab("week") +
  ylab("Mean Abundance (log scale)") + 
  labs(color= expression("Predicted counts of " *italic("C. obsoletus")* " and " *italic(" C. scoticus")* " by year")) + 
  theme(legend.position="bottom") + 
...
df_cv_abundance2 <- df_cv_abundance_LTO %>% ## separation of prediction in different groups according to the number predicted to better wizualisation
  mutate(obs = exp(obs)) %>%
...
imp <- imp %>%
  ...
  arrange(-importance) %>% 
  mutate(Type = case_when(var %in% c("TX_0_0", "FG_0_1") ~ "Meteorological",
                          var %in% c("SWV_0_3", "NDVI_0_5") ~ "Environmental",
                          var %in% ("CLC_level2") ~ "Landscape")) %>%
  mutate(var = case_when(var == "TX_0_0" ~ "Average Max Temperature 1 week before collection",
                         var == "SWV_0_3" ~ "Soil moisture 1-4 weeks before collection",
                         var == "NDVI_0_5" ~ "Vegetation Index 1-6 weeks before collection",
                         var == "CLC_level2" ~ "Land use",
                         var == "FG_0_1" ~ "Avg. wind strength 1-2 weeks before collection"))
...
plot_pdp_2D <- ggplot(pd, aes(x = TX_0_0, y = ALT, fill = exp(yhat))) +
  geom_tile() +
