...
#########singlular ROC plot for all zones
df_cv_presence_LTO <- res_model_presence_LTO_femelles[[2]]  #### data frame with prediction # %>% #  left_join(grid)
...
roc_plot_func <- function(df, var){
  ...
  for (i in unique(df_new[[var]])) {  
    ...
    LLO_scores <- df_temp$pred
    
    i_mmdat <- mmdata(LLO_scores, df_temp$obs, modnames=("LLO"))
    AUC_i_LLO <- MLmetrics::AUC(df_temp$pred, df_temp$obs)
    i_obj <- precrec::evalmod(i_mmdat)
    roc_plot <- autoplot(i_obj, curvetype= c("ROC"))+
      geom_line(linewidth=1)+
...
roc_plots <- roc_plot_func(df_cv_presence_LTO, "ECO_CLI")
roc_plots_all <- patchwork::wrap_plots(roc_plots, ncol = 3) + plot_annotation(title = "Presence Model Evaluation\n(>5 FEMELLES PARES)")
