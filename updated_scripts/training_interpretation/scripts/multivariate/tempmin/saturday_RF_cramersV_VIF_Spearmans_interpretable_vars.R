...
write.csv(df_model, "./updated_scripts/df_model_for_RF.csv", row.names = F)
...
df_model_abundance <- df_model %>%
  ...
  mutate(across(ID_SITE:year, as.factor))%>%
  filter(!is.na(NDVI_0_5), !is.na(VENTDEBUT), !is.na(FG_0_1))
...
saveRDS(res_multiv_model_abundance_LLO, "./updated_scripts/models/reduced_RF_model_abundance_LLO_interpretation_alt_vent.rds")
...
saveRDS(res_multiv_model_abundance_LTO,"./updated_scripts/models/reduced_RF_model_abundance_LTO_interpretation_alt_vent.rds")
