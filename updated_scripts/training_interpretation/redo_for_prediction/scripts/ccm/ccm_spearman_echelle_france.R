...
df_model <- read.csv(file.path("./data","df_to_model_obscot_femelles_pares_indiv.csv")) %>%
...
df_model <-  df_model %>%
  mutate(PRESENCE_OBSCOT = ifelse(NBINDIV<1,0,1),
         PRESENCE_FEMELLLES = ifelse(NBFEMELLES<5,0,1),
         PRESENCE_PARES  = ifelse(NBFEMELLES<5,0,1)) %>% 
  relocate(PRESENCE_OBSCOT, PRESENCE_FEMELLLES, PRESENCE_PARES, .after = NBFEMELLES) %>%
  filter(!is.na(PRESENCE_OBSCOT), !is.na(PRESENCE_FEMELLLES), !is.na(PRESENCE_PARES)) %>%
  as_tibble()
...
fun_compute_correlation_univ <- function(df,indicator){
  ...
  if(indicator == "presence"){
    #var_to_keep = "PRESENCE_OBSCOT"
    #var_to_keep = "PRESENCE_FEMELLES"
    var_to_keep = "PRESENCE_PARES"
  ...
  spearman_univs <- furrr::future_map(colnames(df[8:ncol(df)]), possible_a)
...
# presence (long run...)#for spearman we dont mutate r values bcz they range from 0 to 1
corr_univ_presence  <- fun_compute_correlation_univ(df_model, "presence") %>%
  as.tibble() %>%
  mutate(indicator = "presence") 

# abundance  (long run...)
corr_univ_abundance <- fun_compute_correlation_univ(df_model, "abundance") %>%
  as.tibble() %>%
...
#p_meteo_presence + plot_annotation(title="Spearman's CCM for C. Obsoletus/ C. Scoticus \n Presence/Absence  (France, with >1 NBINDIV)",
#p_meteo_presence + plot_annotation(title="Spearman's CCM for C. Obsoletus/ C. Scoticus \n Presence/Absence  (France, with >5 NBFEMELLES)",
 p_meteo_presence + plot_annotation(title="Spearman's CCM for C. Obsoletus/ C. Scoticus \n Presence/Absence  (France, with >5 NBPARES)",
                                     theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14)))

p_meteo_abundance <- patchwork::wrap_plots(plots_univ_spearman_temporal_mf$univ_temporal[2], ncol = 1  , nrow = 1)
p_meteo_abundance + plot_annotation(title="Spearman's CCM for C. Obsoletus/ C. Scoticus Abundance\n  (France, with >1 NBINDIV)",
                                   theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14)))
...
write.csv(univ_spearman_temporal_mf_test, "./data/spearman_ccm_output_presence_pares.csv", row.names=F)
