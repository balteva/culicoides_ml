...
raw_data <- read.csv("./data/Landscape_Micro_obscot_femelles_pares_df.csv") %>%
  select(-c(DATEDETERMINATIONDEF, OBSERVATIONSDIVERSES, CHGTEMPLACEMENT, ESPECE))%>%
...
df_model <- data %>%
  mutate(across(ID_SITE:ECO_CLI, as.factor),
         ...
         NBPARE=as.numeric(NBPARE),
         NBFEMELLES=as.numeric(NBFEMELLES),
  ...
  mutate(
    PRES_OBSCOT = ifelse(NBINDIV > 1, 1, 0),
    PRES_FEMELLES = ifelse(NBFEMELLES> 5, 1, 0),
    PRES_PARES = ifelse(NBPARE > 5, 1, 0),
  ...
  relocate(NBINDIV,NBPARE, NBFEMELLES, PRES_OBSCOT, PRES_FEMELLES, PRES_PARES, .after=ID_SITE)%>%
...
predictors <- setdiff(colnames(df_model), c( "NBINDIV","NBPARE", "NBFEMELLES","PRES_OBSCOT", "PRES_FEMELLES", "PRES_PARES",  "ID_SITE", 'DATE', "ECO_CLI", "num_releve","idpointdecapture", "longitude", "latitude")) ## Selecting all the columns which are not data on presence/abundance and metadata as predictors

  
  df_glmm <- df_model %>% 
  dplyr::select(NBINDIV,PRES_PARES,ID_SITE,DATE,predictors) %>% ## Selecting only both response variables + 2 random effects (ID_SITE and DATE) + predictors  
  mutate(NBINDIV= as.character(NBINDIV), PRES_PARES = as.character(PRES_PARES)) %>%
  ...
  mutate(NBINDIV = as.numeric(NBINDIV), PRES_PARES= as.numeric(PRES_PARES)) 
...
fun_compute_glmm_univ <- function(df_glmm, indicator) {
  ...
  if (indicator == "presence") { 
    func <- function(x) {
      ret <- glmmTMB(as.formula(paste0("PRES_PARES ~ ", x, " + (1|ID_SITE) + (1|DATE)")), 
...
glmm_univ_presence2 <- fun_compute_glmm_univ(df_glmm, "presence")
write.csv(glmm_univ_presence2,'./data/glmm_presence_obscot_5pares.csv', row.names = F)

summary(glmm_univ_presence[[1]])


##### For abundance models: code make take long to run (~10 min)
df_glmm <- df_glmm %>% filter(NBINDIV>1) ## Selecting only the strictly positive number 
glmm_univ_abundance <- fun_compute_glmm_univ(df_glmm, "abundance")
write.csv(glmm_univ_abundance,'./data/glmm_abundance_obscot_1nbindiv.csv', row.names = F)


write.csv(df_model, './data/cleaned_landscape_vars_femelles_pares_nbindiv.csv', row.names = F)