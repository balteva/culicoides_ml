...
cramersv_presence_vars <- c("SITUATIONPIEGE", "ACTIV_VIANDE") #VENTEBUT was found to be corr to altitude
...
filter(!is.na(OVI), !is.na(BETAIL), !is.na(ALT), !is.na(TEMPERATUREDEBUT), !is.na(TEMPERATUREFIN), !is.na(TEMPMINI), !is.na(TEMPMAX), !is.na(EVI_0_3))
...
df_model_presence <- df_model %>%
  ...
  filter(!is.na(SITUATIONPIEGE), !is.na(ACTIV_VIANDE), !is.na(EVI_0_3))
...
saveRDS(res_multiv_model_presence_LLO,"./updated_scripts/models/RF_model_presence_LLO_interpretation.rds")
...
saveRDS(res_multiv_model_presence_LTO,"./updated_scripts/models/RF_model_presence_LTO_interpretation.rds")
