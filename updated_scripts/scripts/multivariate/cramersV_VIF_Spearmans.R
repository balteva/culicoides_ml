...
micro_landscape_df<- read.csv('../../updated_scripts/data/df_mandscape_for_multivariate.csv')%>%




# loading meteo vars
ccm_meteo <- read.csv("../../data/obscot/df_to_model_fixed_obscot.csv")%>%
...
categ_presence_vars <- c("SITUATIONPIEGE", "ACTIV_VIANDE", "OUVERTURE_BAT", "PLUIEDEBUT", "VENTDEBUT", "VENTFIN") #selecting only categorical vars for presence for chi square test

categ_presence_df <- df_combined %>%
  select(categ_presence_vars)%>% 
  na.omit()%>%
...
fishers_func_categorical(categ_presence_df, categ_presence_vars, "../../updated_scripts/data/significance_tests/obscot_presence_fishers.txt")
...
cramersv_presence_vars <- c("SITUATIONPIEGE", "VENTDEBUT", "ACTIV_VIANDE")

##### Numerical var selection for presence model
vars_presence_numerical <- c("TM_0_0", "TM_0_2", "TN_0_0", "TN_0_1", "TX_0_4",
                             "UM_0_0", "TPS_0_0", "TPS_0_2", "QQ_1_6", "QQ_1_5", "PP_5_5", "PP_0_5",
                             "ALT", "OVI", "BETAIL","TEMPERATUREDEBUT", "TEMPERATUREFIN", "TEMPMINI", "TEMPMAX") #"TEMPERATUREDEBUT", "TEMPERATUREFIN", "TEMPMINI", "TEMPMAX" removing these for purely predictive model

#if I remove these vars : "TEMPERATUREDEBUT", "TEMPERATUREFIN", "TEMPMINI", "TEMPMAX", the VIF method wont select temp vars at all

#because they are good predictors, I will replace TEMPMAX and TEMPMINI with TN_0_0 and TX_0_4 in the future
df_model_numerical <- df_model %>%
filter(!is.na(OVI), !is.na(BETAIL), !is.na(ALT), !is.na(TEMPERATUREDEBUT), !is.na(TEMPERATUREFIN), !is.na(TEMPMINI), !is.na(TEMPMAX))
...
#selects "UM_0_0"         "ALT"            "OVI"            "BETAIL"          "TEMPMINI"      
...
filtered_numer_presence_vars <- c("UM_0_0", "ALT","OVI","BETAIL","TEMPMINI")
#basically, if i include TEM
#############################       vISUALISATION
...
ggplot(melted_m, aes(Var1, Var2, fill = value)) +
  ...
  labs(title = "Correlation Heatmap for Numerical Variables in Obs/Scot Abundance model")
#################################################### END OF VIZ
############finished here
...
predictors_presence <-  c(filtered_numer_presence_vars, cramersv_presence_vars)
...
df_model_presence <- df_model %>%
  ...
  filter(!is.na(SITUATIONPIEGE), !is.na(VENTDEBUT), !is.na(ACTIV_VIANDE), !is.na(TEMPMINI))
...
saveRDS(res_multiv_model_presence_LLO,"../../updated_scripts/models/RF_model_presence_LLO.rds")
...
saveRDS(res_multiv_model_presence_LTO,"../../updated_scripts/models/RF_model_presence_LTO.rds")
...
df_model_numerical <- df_model %>%
  filter(!is.na(ALT), !is.na(TEMPERATUREDEBUT), !is.na(TEMPERATUREFIN), !is.na(TEMPMINI), !is.na(TEMPMAX))
...
#output [1] "UM_0_0"   "EVI_1_5"  "TEMPMINI" "ALT"
...
#output : "UM_0_0"   "PP_3_3"   "EVI_1_5"  "TEMPMINI" "TEMPMAX"  "ALT"


filtered_numer_abundance_vars <- c("UM_0_0",   "EVI_1_5",  "TEMPMINI", "ALT")
...
df_model_abundance <- df_model %>%
  mutate(year = year(as.Date(DATE)))%>%
  filter(NBINDIV>0) %>%
  dplyr::select("idpointdecapture", "ID_SITE","DATE","Cell", "year", "NBINDIV", "PRES_CUL", predictors_abundance)%>%
  mutate(across(ID_SITE:year, as.factor))%>%
  filter(!is.na(ELEV_OVIN), !is.na(CLC_level2), !is.na(VENTDEBUT), !is.na(PLUIEDEBUT), !is.na(TEMPMINI))
...
saveRDS(res_multiv_model_abundance_LLO, "../../updated_scripts/models/RF_model_abundance_LLO.rds")
...
saveRDS(res_multiv_model_abundance_LTO,"../../updated_scripts/models/RF_model_abundance_LTO.rds")
