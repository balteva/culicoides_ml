...
multiv_model_abundance_LLO <- readRDS("./updated_scripts/models/RF_model_abundance_LLO.rds")


#CV with LTO
multiv_model_abundance_LTO <- readRDS("./updated_scripts/models/RF_model_abundance_LTO.rds")
...
plot_eval_abundance_model <- df_cv_abundance_LLO %>%
  ...
  facet_wrap(~year, ncol=2) + #~ECO_CLI
  theme_bw() + 
...
# And instead I compare the raw outputs
  df_metrics_perf <- df_cv_abundance_LTO  %>% 
  mutate(year = year(as.Date(DATE))) %>%
