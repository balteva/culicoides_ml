...
###average activity period for each month
...
trial_start_of_season_avg <-  joined_data %>%
  group_by(year,ID_SITE,longitude, latitude)%>% 
  mutate(start = ifelse (any (prediction_final >= 1), min(week[prediction_final >= 1], na.rm=TRUE), NA))%>%
  group_by(ID_SITE,longitude, latitude)%>%
  summarise(median_start=median(start), avg_start=mean(start))%>%
  ungroup() %>%
  select(c(ID_SITE,longitude, latitude,median_start, avg_start))
...
trial_start_of_season<- joined_data %>%
  group_by(year, ID_SITE,longitude, latitude)%>% 
  mutate(first_week = ifelse (any (prediction_final >= 1), min(week[prediction_final >= 1], na.rm=TRUE), NA))%>%
  ungroup() %>%
  select(year, ID_SITE,longitude,latitude, prediction_final, first_week)%>%
  distinct() %>%
  left_join(trial_start_of_season_avg, by=c("ID_SITE", "longitude", "latitude"))%>%
  mutate(error_weeks_avg = first_week - avg_start, error_weeks_median= first_week - median_start) %>%
...
#diverging for presence absence ['#b2182b','#ef8a62','#fddbc7','#d1e5f0','#67a9cf','#2166ac']
# trial_start_of_season1 <-trial_end_of_season %>%
#   filter(is.na)
