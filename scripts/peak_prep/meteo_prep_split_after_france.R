...
pieges_data <- read.csv("C:/Users/ibalt/OneDrive/Desktop/uni/M2 stage/Cullicoides_data/CIRAD/culicoides_ml/scripts/peak_prep/data/france_after_peak.csv") %>%
  select(-X) %>%
  rename(date = DATE) %>%
  mutate(num_releve = seq(1,nrow(.),1)) %>%
  mutate(idpointdecapture = paste0(ID_SITE,"_",num_releve))
...
meteo <- read_delim(file.path("../../data","meteo_df.csv")) %>%
  as_tibble() %>%
  rename(date = DATE) %>%
...
lag_max <- 30 # (1 mois avant la capture)
...
fun_summarize_week <- function(df_meteo_pieges2,var_to_summarize,fun_summarize,new_var_name,n_days_agg){
  ...
  if(fun_summarize=="sum"){
    df_meteo_pieges2_summarize <- df_meteo_pieges2 %>%
      ...
      #group_by(idpointdecapture, lag_n = lubridate::week(date), year = lubridate::year(date)) %>%    ##inesvtigate wtf
...
df_meteo_pieges2 <- df_meteo_pieges %>%
  left_join(meteo, by = c("date","ECO_CLI","ID_SITE")) %>%
...
df_meteo_pieges_summ <- df_meteo_pieges_summ %>% filter(lag_n<4)
...
write.csv(df_to_model,file.path("../../data","df_to_model_after.csv"), row.names = F)
