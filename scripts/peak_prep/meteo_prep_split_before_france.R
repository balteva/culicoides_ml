...
pieges_data <- read.csv("C:/Users/ibalt/OneDrive/Desktop/uni/M2 stage/france_before_peak.csv") %>%
  select(-X) %>%
  rename(date = DATE) %>%
...
meteo <- read_delim(file.path("../2025_ieva_culicoides/data","meteo_df.csv")) %>%
  as_tibble() %>%
  rename(date = DATE) %>%
...
write.csv(df_to_model,file.path("../2025_ieva_culicoides/data","df_to_model_before.csv"), row.names = F)
