...
df_model <- read.csv(file.path("../2025_ieva_culicoides/data","df_to_model.csv")) %>%
...
corr_univ_abundance <- df_model %>%
  ...
  # function to plot the CCM (SPEARMANS) (Tutors code with issue)
  # fun_ccm_plot2 <- function(correlation_df, var){
  #   
  #   correlation_df$abs_corr <- abs(correlation_df$correlation) #calculating the absoluute correlation, stoting in com abs_corr
  #   
  #   if(!is.na(unique(correlation_df$abs_corr)[1])){#checks if at least one value is non NA in abs_corr
  #     most_corr <- correlation_df %>% filter(abs_corr == max(abs_corr, na.rm = T)) #finds highest corr value
  #     most_corr2 <- correlation_df %>% arrange(desc(abs_corr)) %>% filter(abs_corr >= most_corr$abs_corr * 0.9) #sorts in descending order and filters rows where the values are at least 90% of the highest corr value
  #   } else { 
  #     most_corr <- most_corr2 <- correlation_df[1,] # to deal with case all correlation values are NAs: if all values are NA, choses first row to represent
  #   }
  #   
