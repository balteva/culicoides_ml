...
pieges_data <- read.csv("../../data/culicoides_df_fixed_obscot.csv") %>%
  dplyr::select(-X) %>%
  rename(date = DATE) %>%
...
meteo_fix <- meteo %>%
  mutate(NDVI = case_when(
    NDVI == -3000 ~ -NA,TRUE ~ NDVI),
    EVI = case_when(
     EVI == -3000 ~ -NA, TRUE ~ EVI))
...
fun_summarize_week <- function(df_meteo_pieges2,var_to_summarize,fun_summarize,new_var_name,n_days_agg){
  ...
  if(fun_summarize=="sum"){###this aggregates selected leteirilogical variale over a selected time period (gives sum values within each period)
...
write.csv(df_to_model,file.path("../../data","df_to_model_fixed_obscot.csv"), row.names = F)
